...
class TestApiSaleWallet < Test

  def test_api_sale_wallet
    ...
    # no payload
    post '/api/sale-wallet'
    assert last_response.unprocessable?
    assert_equal 12, JSON.parse(last_response.body).keys.size

    # invalid access_token
    post '/api/sale-wallet', { amount: amount, currency: currency, access_token: 'any', digest: 'any',
    ...
    post '/api/sale-wallet', { amount: amount, currency: currency, access_token: terminal.access_token, digest: digest,
                          systan: systan, api_version: '1.0.0.', application_version: '1.0.0.', card_number: 'invalid', card_pin: card_pin,
                          client_datetime: client_datetime, cashier_pin: cashier_pin, cashier_id: cashier.id}.to_json
    assert last_response.unprocessable?
    assert JSON.parse(last_response.body)['digest']

    # invalid target account by currency
    post '/api/sale-wallet', { amount: amount, currency: 'EUR', access_token: terminal.access_token, digest: digest,
                          systan: systan, api_version: '1.0.0.', application_version: '1.0.0.', card_number: card_number, card_pin: card_pin,
                          client_datetime: client_datetime, cashier_pin: cashier_pin, cashier_id: cashier.id}.to_json
    assert last_response.unprocessable?
    ...
    post '/api/sale-wallet', { amount: amount, currency: currency, access_token: terminal.access_token, digest: digest,
                          systan: systan, api_version: '1.0.0.', application_version: '1.0.0.', card_number: card_number, card_pin: '1111',
                          client_datetime: client_datetime, cashier_pin: cashier_pin, cashier_id: cashier.id}.to_json
    assert last_response.unprocessable?
    ...
    # sale now
    wallet_account.update balance: amount + 1 # extra one cent, amount must be greater than available balance
    payload = { amount: amount, currency: currency, access_token: terminal.access_token, digest: digest,
                systan: systan, api_version: '1.0.0.', application_version: '1.0.0.', card_number: card_number, card_pin: card_pin,
                client_datetime: client_datetime, cashier_pin: cashier_pin, cashier_id: cashier.id}.to_json
    post '/api/sale-wallet', payload
    ...
    # check user account
    assert_equal 1, wallet_account.reload.balance  # debit for amount, one cent left
    ...
    assert_equal account.balance + amount, account.reload.balance  # credit for amount

    # merchant transaction
    merchant_trx = account.transactions.first
    assert_equal amount, merchant_trx.amount
    assert_equal 'sale_wallet_mpos', merchant_trx.transaction_type # check fields
    ...
    # user transaction
    user_trx = wallet_account.transactions.first
    assert_equal amount, user_trx.amount
    assert_equal wallet_account.balance, user_trx.balance # reflect balance in trx
    assert_equal 'sale_wallet', user_trx.transaction_type # check fields
    assert_equal 'debit', user_trx.type
    ...
    post '/api/sale-wallet/retry', payload
    assert last_response.ok?
